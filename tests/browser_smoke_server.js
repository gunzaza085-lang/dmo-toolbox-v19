...
http.createServer((req,res)=>{
  ...
  if(url.pathname==='/config.js'){res.writeHead(200,{'Content-Type':'text/javascript;charset=utf-8','Cache-Control':'no-store'});return res.end(`window.DMO_CONFIG=${JSON.stringify({sheetsUrl:`http://127.0.0.1:${port}/api`,refreshMs:600000,appVersion:'LOCAL-SMOKE'})};`);}
  const file=path.resolve(root,url.pathname==='/'?'index.html':url.pathname.slice(1));if(!file.startsWith(root)){res.writeHead(403);return res.end('Forbidden');}
...
}).listen(port,'127.0.0.1',()=>console.log(`Local smoke server ready at http://127.0.0.1:${port}`));
